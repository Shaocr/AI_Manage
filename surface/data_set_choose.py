...
import os
#编辑节点的html文件位置
html_path = os.path.dirname(os.path.realpath(__file__))+'\\dag\\page\\data.html'
#导入主控制台
from control.main_control import console
#主窗口
class Data_Choose(QWidget):
    #给以后留的接口
    #向html发送数据用的信号
    SigSendMessageToJS = pyqtSignal(str)
    def __init__(self):
        super(Data_Choose,self).__init__()
        self.resize(720,480)
        #浏览器
    ...
    def OnReceiveMessageFromJS(self, strParameter):
        ...
        if not strParameter:
            return
        console.get_data_from_dataset(int(strParameter))

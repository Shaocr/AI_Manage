...
import os
#编辑节点的html文件位置
html_path = os.path.dirname(os.path.realpath(__file__))+'\\dag\\page\\dynamic.html'
#导入主控制台
from control.main_control import console
#主窗口
class Dynamic_line(QWidget):
    #给以后留的接口
    #向html发送数据用的信号
    SigSendMessageToJS = pyqtSignal(str)
    def __init__(self):
        super(Dynamic_line,self).__init__()
        self.resize(720,480)
        #浏览器
        ...
        self.button = QPushButton("123")
        self.button.clicked.connect(self.OnSendMessageByInteractObj)
        #self.browser.load(QUrl('https://www.baidu.com'))
        #self.setCentralWidget(self.browser)

    def OnReceiveMessageFromJS(self, strParameter):
        print('OnReceiveMessageFromJS('+strParameter+')')
        self.OnSendMessageByInteractObj('123')
    def OnSendMessageByInteractObj(self,data='123'):
        self.SigSendMessageToJS.emit(data)
